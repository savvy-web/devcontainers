...
`package-manager` declares `"installsAfter": ["ghcr.io/savvy-web/features/node"]`.
...
1. **`discover`** job — runs `node .github/scripts/topo-order.js` with
   `SKIP_REGISTRY_CHECK=true` to build a matrix of feature entries
   (`{ id, version, path, deps, skipAutogenerated, ... }`)
2. **`test`** job — fan-out matrix; installs `@devcontainers/cli`; runs
   `bash .github/scripts/test-feature-isolated.sh <id>`. That script copies
   `src/` and `test/` into a scratch directory, strips
   `ghcr.io/savvy-web/features/*` entries from `installsAfter` (the CLI
   rejects 3-segment OCI references it cannot resolve), then runs
   `devcontainer features test -f <id> -i <base-image> [--skip-autogenerated] --skip-duplicated <scratch>`.
...
`publish-features.yml` reuses `test-feature-isolated.sh` for its in-line test
step before each `devcontainer features publish` call, walking features in
topological order so dependents see their deps already in the registry.
